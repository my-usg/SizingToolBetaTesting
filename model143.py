...
# ── page config ───────────────────────────────────────────────────────────────
st.set_page_config(page_title="Model 143 Sizing Tool", page_icon="⚙️", layout="wide")

st.title("⚙️ Model 143 Sizing Tool")
...
_code   = "".join(_lines[:445])   # stop before INPUT section
...
def build_table(prefix, opp_type, result143):
    ...
    for reg, cap in result143.items():
        ...
        if opp_type == "IRV":
            irv = _globals["will_irv_work143"](reg)
