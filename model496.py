...
_code   = "".join(_lines[:418])   # stop before INPUT section
...
def build_table(prefix, opp_type, result496):
    ...
    for reg, cap in result496.items():
        ...
        if opp_type == "IRV":
            irv = _globals["will_irv_work496"](reg)
...
errors = []
if inlet_input > 0 and (_inlet_psi_check > 125 or _inlet_psi_check < 1):
    errors.append("Inlet pressure must be between 1 and 125 psi.")
if outlet_input > 0 and (_outlet_psi_check < 3.5/28 or _outlet_psi_check > 2):
