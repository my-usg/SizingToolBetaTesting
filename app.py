...
with st.sidebar:
    ...
    st.subheader("Units")
    inlet_units    = st.selectbox("Inlet pressure units",  ["psi", "in wc", "bar"])
    ...
    if errors:
        ...
        with st.spinner("Sizing regulator…"):
            try:
                ...
                if inlet_units == "in wc":
                    inlet_psi = inlet_input * (1/28)
                elif inlet_units == "bar":
